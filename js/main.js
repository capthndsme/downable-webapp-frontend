...
function placeFile(blob, songTitle, songID) {
    db.putAttachment(songID, songTitle, blob, blob.type).then(function(response) {
...
function serverConvert(TubeID, Title) {
    ...
    xhttp.open("GET", "//api.host/DownableAPI@1/api.php?id=" + TubeID + "&title=" + Base64.encode(Title), true);
    xhttp.onload = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (data.Message === "success") {
                ...
                downloadFile("//api-home.capthnds.me/DownableAPI@1/" + data.url,  Title, TubeID);
            }
...
function getBlobArt(id, cb) {
    imgdb.getAttachment(id + '-album', id + '-album').then(function (blobOrBuffer) {
...
function downloadFile(url, songTitle, songID) {
    ...
    oReq.onprogress = function(evt) {
        ...
        dx.parentNode.parentNode.getElementsByClassName("DownloadSize")[0].innerHTML = humanFileSize(evt.loaded, true) + " / " + humanFileSize(evt.total, true);
    ...
    oReq.onload = function(oEvent) {
        ...
        if (oReq.status == 200) {
            ...
            downloadArt(songID);
            document.getElementById(songID + "-dlbar").parentNode.parentNode.getElementsByClassName("DownloadSize")[0].innerHTML = "Download Complete!";
            QueueManager.addSong(songTitle, songID);
            var xxx = QueueManager.findPositionInAllSongs(songID);
            createSongElement(QueueManager.AllSongsEx[xxx][0], QueueManager.AllSongsEx[xxx][1], xxx);
...
function listAllFiles(cb) {
    db.allDocs({
        ...
        for (i = 0; i < QueueManager.AllSongsEx.length; i++) {
            createSongElement(QueueManager.AllSongsEx[i][0], QueueManager.AllSongsEx[i][1], i)
        };
...
function songMore(ref, event) {
    ...
    Swal.fire({
        ...
        showOKButton: false,
        html: "Actions: <div data-songtitle='" + sTitle + "' data-songid='" + sID + "' data-arrayindex='" + arrIn + "' data-playlistID='" + plist + "' class='songOptions' ><br/><button class='moreSelect' onclick='addSongToPlaylist(this.parentElement)'>Add to playlist</button><br/><button class='moreSelect' onclick='addSongToQueue(this.parentElement)'>Add to Queue</button><br/><button class='moreSelect' onclick='deleteSongAndConfirm(this.parentElement)'>Delete</button></div>",
    }).then((result) => {
        console.log(result);
    });
}
...
function addSongToPlaylist(ref) {
    swal.clickConfirm();
    window.addSongToPlaylistRef = [ref.getAttribute("data-songtitle"), ref.getAttribute("data-songid")];
...
function placeFile(blob, artid) {
    imgdb.putAttachment(artid, artid, blob, blob.type).then(function(response) {
        console.log(response);
    }).catch(function(err) {
        console.log(err);
    });
...
// songTitle & songID for Metadata only. 
function downloadblob(url, artid) {
    let oReq = new XMLHttpRequest();
    oReq.open("GET", url, true);
    oReq.responseType = "blob";
    oReq.onload = function(oEvent) {
        ...
        if (oReq.status == 200) {
            let blob = oReq.response;
            placeFile(blob, artid);
            localStorage.setItem(artid, "yes")
        } else if (oReq.status == 404) {
            localStorage.setItem(artid, 'no-album');
        }
    };
...
function downloadArt(vidid) {
    console.log("dlart");
    var xhttp = new XMLHttpRequest();
    xhttp.open("GET", "//api.host/DownableAPI@1/getimage.php?id=" + vidid, true);
    xhttp.onload = function() {
        if (this.readyState == 4 && this.status == 200) {
            downloadblob(xhttp.responseText, vidid + '-album')
        } else if (this.status == 500) {
            setTimeout(function() {
                downloadArt(vidid);
            }, 1500);
...
function callDownload(ref) {
    ...
    serverConvert(elem.getAttribute("data-download"), elem.getAttribute("data-title"));
}
...
function DownloadResultReady(p) {
    ...
    serverConvert(p.getAttribute("data-download"), p.getAttribute("data-title"));
    }
}
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (data.error) {
                ...
                document.getElementById("DownloadResultReady").setAttribute("data-download", YouTubeGetID(ref));
                document.getElementById("DownloadResultReady").setAttribute("data-title", data.title);
                document.getElementById("DownloadResultReady").setAttribute("data-provider", "youtube");
...
function doSearch(SEARCH_TERM, NO_RETRY) {
    ...
    xhttp.open("GET", "//api.host/YouTubeSearch@1/api/search?q=" + SEARCH_TERM, true);
    xhttp.onload = function() {
        if (this.readyState == 4 && this.status == 200) {
            let data = JSON.parse(xhttp.responseText).results;
            for (var i = 0, len = data.length; i < len; i++) {
              
                var $dlList = document.getElementById("tmp_SearchResult").cloneNode(true);
                $dlList.id = "dl-" + data[i].video.id;
                $dlList.setAttribute("data-download", data[i].video.id);
                $dlList.setAttribute("data-title", data[i].video.title);
                $dlList.getElementsByClassName("searchResultImg")[0].src= data[i].video.thumbnail_src;
                $dlList.getElementsByClassName("searchResultTitle")[0].innerHTML= data[i].video.title;
                $dlList.getElementsByClassName("searchResultMeta")[0].innerHTML= data[i].uploader.username + "<br/>" + data[i].video.snippet;
...
function do_Load() {
    ...
    FullRange.ontouchstart = function() { notDragging = false; }
...
}('//api-home.capthnds.me/ErrorLogger/logerrors.php'));
...
function filterManager(ref) {
    if (ref.value.length >= 1) {
        ...
        for (var i = 0; i < QueueManager.Queue.length; i++) {
            if (new String(QueueManager.Queue[i][0]).toUpperCase().includes(searchTerm)) {
                createSongSearchedElement(QueueManager.Queue[i][0], QueueManager.Queue[i][1], i);
            } 
        }
    } else {
